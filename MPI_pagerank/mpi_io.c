              ...
              MPI_Comm comm)    /* (IN) MPI communicator */
{
    ...
    {
        printf
            ("Proc:%d is writing rank values of %d urls to file pagerank.output\n",
             rank, totalNumUrls);
        sprintf(outFileName, "pagerank.output");
        ...
        for (i = 0; i < totalNumUrls; i++)
        {
            ...
            sprintf(str, "%f ", rank_values_table[i]);
            MPI_File_write(fh, str, strlen(str), MPI_CHAR, &status);
            MPI_File_write(fh, "\n", 1, MPI_CHAR, &status);
        }
        MPI_File_close(&fh);
